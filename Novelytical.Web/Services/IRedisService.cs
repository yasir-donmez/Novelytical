...
public interface IRedisService
{
    ...
    Task<string?> GetStringAsync(string key); // Added string getter
    Task SetAsync(string key, long value, TimeSpan? expiry = null);
    Task SetAsync(string key, string value, TimeSpan? expiry = null); // Added string overload
    ...
    Task<long> SortedSetLengthAsync(string key); // Added for pagination count
}
